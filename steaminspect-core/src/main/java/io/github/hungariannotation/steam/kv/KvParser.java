...
import io.github.hungariannotation.steam.kv.KvToken.*;
...
public class KvParser {
     ...
     * @throws IOException
     */
    public static KvValue parse(String string) {
        try {
            return parse(string, KvString::new, KvTable::new);
        } catch (IOException e) {
            throw new UncheckedIOException("IOException while parsing string", e);
        }
    }
     ...
     * @throws IOException
     */
    public static KvValue parse(KvTokenStream stream) throws IOException {
        return parse(stream, KvString::new, KvTable::new);
    }
    ...
    public static KvValue parse(byte[] byteArray, Charset utf8) throws IOException {
        try (Reader rd = new InputStreamReader(new ByteArrayInputStream(byteArray), utf8)) {
            return parse(rd);
        }
    }
}
